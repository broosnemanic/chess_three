class_name Array2DSquareType

var _internal: Array[Lists.SQUARE_TYPE]			# Linear list of all items for internal use
var size: Vector2i						# [column count, row count]
var type: Lists.SQUARE_TYPE						# For constructing null type and validating array arguments
...
func construct_from_rows(a_row_set: Array[Array]):
	...
	for i_row: Array in a_row_set:
		if not i_row is Array[Lists.SQUARE_TYPE]: 
			return
	...
	for i_row_index: int in range(a_row_set.size()):
		# This assignment is to appease replace_row() which expects an Array[Lists.SQUARE_TYPE]
		var t_row: Array[Lists.SQUARE_TYPE] = a_row_set[i_row_index]
...
func construct_from_columns(a_column_set: Array[Array]):
	...
	for i_column: Array in a_column_set:
		if not i_column is Array[Lists.SQUARE_TYPE]: 
			return
	...
	for i_column_index: int in range(a_column_set.size()):
		# This assignment is to appease replace_column() which expects an Array[Lists.SQUARE_TYPE]
		var t_column: Array[Lists.SQUARE_TYPE] = a_column_set[i_column_index]
...
func construct_from_linear(a_linear: Array[Lists.SQUARE_TYPE], a_size: Vector2i):
...
func linear() -> Array[Lists.SQUARE_TYPE]:
	return _internal
...
func row(a_row_index: int) -> Array[Lists.SQUARE_TYPE]:
...
func column(a_column_index: int) -> Array[Lists.SQUARE_TYPE]:
...
func slice(a_origin: Vector2i, a_direction: Vector2i) -> Array[Lists.SQUARE_TYPE]:
	if not is_valid_coord(a_origin): return []
	var t_direction = normalized_vector(a_direction)
	if t_direction == Vector2i(0, 0): return []
	var t_slice: Array[Lists.SQUARE_TYPE] = [at(a_origin)]
...
func at(a_coord: Vector2i) -> Lists.SQUARE_TYPE:
	return at_coord(a_coord.x, a_coord.y)


# Item at given column and row
func at_coord(a_column: int, a_row: int) -> Lists.SQUARE_TYPE:
...
func append_row(a_row: Array[Lists.SQUARE_TYPE]):
	...
	var t_row: Array[Lists.SQUARE_TYPE] = a_row.duplicate()
	t_row.resize(size.x)
	size = Vector2i(size.x, size.y + 1)
	for i_item: Lists.SQUARE_TYPE in t_row:
		_internal.append(i_item)


func append_column(a_column: Array[Lists.SQUARE_TYPE]):
	# We are going to resize the added array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new column is too tall the extra items are ignored
	var t_column: Array[Lists.SQUARE_TYPE] = a_column.duplicate()
	t_column.resize(size.y)
	var t_internal: Array[Lists.SQUARE_TYPE] = []
...
func put(a_item: Lists.SQUARE_TYPE, a_coord: Vector2i):
	put_at_coord(a_item, a_coord.x, a_coord.y)


# Replaces item at [a_column, a_row] with a_item
func put_at_coord(a_item: Lists.SQUARE_TYPE, a_column: int, a_row: int):
	_internal[a_column + a_row * size.x] = a_item


# As it says
func replace_row(a_row_index: int, a_new_row: Array[Lists.SQUARE_TYPE]):
	# We are going to resize the new row array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new row is too long the extra items are ignored
	var t_row: Array[Lists.SQUARE_TYPE] = a_new_row.duplicate()
	t_row.resize(size.x)
...
# As it says
func replace_column(a_column_index: int, a_new_column: Array[Lists.SQUARE_TYPE]):
	...
	var t_column: Array[Lists.SQUARE_TYPE] = a_new_column.duplicate()
	t_column.resize(size.y)
...
func out():
	...
	for i_row: Array[Lists.SQUARE_TYPE] in t_rows:
		print_row(i_row)


# String representation of a_row
func print_row(a_row: Array[Lists.SQUARE_TYPE]):
	var t_text: String = ""
	for i_item: Lists.SQUARE_TYPE in a_row:
		t_text += str(i_item) + " "
	print(t_text)
...
func row_out_at_width(a_row: Array[Lists.SQUARE_TYPE], a_width: int) -> String:
	var t_string: String = ""
	var t_spacer: String = ", "
	for i_item: Lists.SQUARE_TYPE in a_row:
