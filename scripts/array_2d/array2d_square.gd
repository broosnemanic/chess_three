class_name Array2DSquare

var _internal: Array[Square]			# Linear list of all items for internal use
var size: Vector2i						# [column count, row count]
var type: Square						# For constructing null type and validating array arguments
...
func construct_from_rows(a_row_set: Array[Array]):
	...
	for i_row_index: int in range(a_row_set.size()):
		# This assignment is to appease replace_row() which expects an Array[Square]
		var t_row: Array[Square] = a_row_set[i_row_index]
...
func construct_from_columns(a_column_set: Array[Array]):
	...
	for i_column: Array in a_column_set:
		if not i_column is Array[Square]: 
			return
	...
	for i_column_index: int in range(a_column_set.size()):
		# This assignment is to appease replace_column() which expects an Array[Square]
		var t_column: Array[Square] = a_column_set[i_column_index]
...
func construct_from_linear(a_linear: Array[Square], a_size: Vector2i):
...
func row(a_row_index: int) -> Array[Square]:
...
func column(a_column_index: int) -> Array[Square]:
...
func at(a_coord: Vector2i) -> Square:
	return at_coord(a_coord.x, a_coord.y)


# Item at given column and row
func at_coord(a_column: int, a_row: int) -> Square:
...
func append_row(a_row: Array[Square]):
	...
	var t_row: Array[Square] = a_row.duplicate()
	t_row.resize(size.x)
	size = Vector2i(size.x, size.y + 1)
	for i_item: Square in t_row:
		_internal.append(i_item)


func append_column(a_column: Array[Square]):
	# We are going to resize the added array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new column is too tall the extra items are ignored
	var t_column: Array[Square] = a_column.duplicate()
	t_column.resize(size.y)
	var t_internal: Array[Square] = []
...
# As it says
func replace_column(a_column_index: int, a_new_column: Array[Square]):
	...
	var t_column: Array[Square] = a_new_column.duplicate()
	t_column.resize(size.y)
...
func out():
	...
	for i_row: Array[Square] in t_rows:
		print_row(i_row)


# String representation of a_row
func print_row(a_row: Array[Square]):
	var t_text: String = ""
	for i_item: Square in a_row:
		if i_item == null:
			t_text += "null, "
		else:
			t_text += i_item.str() + ", "
...
func row_out_at_width(a_row: Array[Square], a_width: int) -> String:
	var t_string: String = ""
