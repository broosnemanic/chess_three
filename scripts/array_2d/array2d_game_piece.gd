class_name Array2DGamePiece

var _internal: Array[GamePiece]			# Linear list of all items for internal use
var size: Vector2i						# [column count, row count]
var type: GamePiece						# For constructing null type and validating array arguments
...
func construct_from_rows(a_row_set: Array[Array]):
	...
	for i_row: Array in a_row_set:
		if not i_row is Array[GamePiece]: 
			return
	...
	for i_row_index: int in range(a_row_set.size()):
		# This assignment is to appease replace_row() which expects an Array[GamePiece]
		var t_row: Array[GamePiece] = a_row_set[i_row_index]
...
func construct_from_columns(a_column_set: Array[Array]):
	...
	for i_column: Array in a_column_set:
		if not i_column is Array[GamePiece]: 
			return
	...
	for i_column_index: int in range(a_column_set.size()):
		# This assignment is to appease replace_column() which expects an Array[GamePiece]
		var t_column: Array[GamePiece] = a_column_set[i_column_index]
...
func construct_from_linear(a_linear: Array[GamePiece], a_size: Vector2i):
...
func linear() -> Array[GamePiece]:
	return _internal


# Array of all items organized into row arrays
...
func row(a_row_index: int) -> Array[GamePiece]:
...
func column(a_column_index: int) -> Array[GamePiece]:
...
func at(a_coord: Vector2i) -> GamePiece:
	return at_coord(a_coord.x, a_coord.y)


# Item at given column and row
func at_coord(a_column: int, a_row: int) -> GamePiece:
...
func append_row(a_row: Array[GamePiece]):
	...
	var t_row: Array[GamePiece] = a_row.duplicate()
	t_row.resize(size.x)
	size = Vector2i(size.x, size.y + 1)
	for i_item: GamePiece in t_row:
		_internal.append(i_item)


func append_column(a_column: Array[GamePiece]):
	# We are going to resize the added array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new column is too tall the extra items are ignored
	var t_column: Array[GamePiece] = a_column.duplicate()
	t_column.resize(size.y)
	var t_internal: Array[GamePiece] = []
...
func put(a_item: GamePiece, a_coord: Vector2i):
	put_at_coord(a_item, a_coord.x, a_coord.y)


# Replaces item at [a_column, a_row] with a_item
func put_at_coord(a_item: GamePiece, a_column: int, a_row: int):
	_internal[a_column + a_row * size.x] = a_item


# As it says
func replace_row(a_row_index: int, a_new_row: Array[GamePiece]):
	# We are going to resize the new row array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new row is too long the extra items are ignored
	var t_row: Array[GamePiece] = a_new_row.duplicate()
	t_row.resize(size.x)
...
# As it says
func replace_column(a_column_index: int, a_new_column: Array[GamePiece]):
	...
	var t_column: Array[GamePiece] = a_new_column.duplicate()
	t_column.resize(size.y)
...
func out():
	...
	for i_row: Array[GamePiece] in t_rows:
		print_row(i_row)


# String representation of a_row
func print_row(a_row: Array[GamePiece]):
	var t_text: String = ""
	for i_item: GamePiece in a_row:
		if i_item == null:
			t_text += "null, "
		else:
			t_text += i_item.out() + ", "
...
func out_at_width(a_width: int):
	var t_rows: Array[Array] = rows()
	for i_row: Array[GamePiece] in t_rows:
		print(row_out_at_width(i_row, a_width))


func row_out_at_width(a_row: Array[GamePiece], a_width: int) -> String:
	var t_string: String = ""
	var t_piece_string: String = ""
	var t_spacer: String = ", "
	for i_item: GamePiece in a_row:
		if i_item == null: 
			t_piece_string = "null"
		else:
			t_piece_string = i_item.out()
		t_piece_string = t_piece_string.substr(0, a_width)	# If too long
		t_piece_string = mpad(t_piece_string, a_width)		# If too short
		t_string += t_piece_string
		t_string += t_spacer
	return t_string

# Returns a left-right padded string if a_string is shorter than a_length
func mpad(a_string: String, a_length: int) -> String:
	if a_string.length() >= a_length: return a_string
	var t_left: int = a_length - (a_length - a_string.length()) / 2	# Note this is int division
	var t_string = a_string.lpad(t_left, "_")
	t_string = t_string.rpad(a_length, "_")
	return t_string
