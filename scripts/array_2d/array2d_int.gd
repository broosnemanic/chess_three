class_name Array2DInt

var _internal: Array[int]			# Linear list of all items for internal use
var size: Vector2i					# [column count, row count]
var type: int						# For constructing null type and validating array arguments
...
func construct_from_rows(a_row_set: Array[Array]):
	...
	for i_row_index: int in range(a_row_set.size()):
		# This assignment is to appease replace_row() which expects an Array[int]
		var t_row: Array[int] = a_row_set[i_row_index]
...
func construct_from_columns(a_column_set: Array[Array]):
	...
	for i_column_index: int in range(a_column_set.size()):
		# This assignment is to appease replace_column() which expects an Array[int]
		var t_column: Array[int] = a_column_set[i_column_index]
...
func construct_from_linear(a_linear: Array[int], a_size: Vector2i):
...
func row(a_row_index: int) -> Array[int]:
...
func column(a_column_index: int) -> Array[int]:
...
func at(a_coord: Vector2i) -> int:
	return at_coord(a_coord.x, a_coord.y)


# Item at given column and row
func at_coord(a_column: int, a_row: int) -> int:
...
func append_row(a_row: Array[int]):
	...
	var t_row: Array[int] = a_row.duplicate()
	t_row.resize(size.x)
	size = Vector2i(size.x, size.y + 1)
	for i_item: int in t_row:
		_internal.append(i_item)


func append_column(a_column: Array[int]):
	# We are going to resize the added array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new column is too tall the extra items are ignored
	var t_column: Array[int] = a_column.duplicate()
	t_column.resize(size.y)
	var t_internal: Array[int] = []
...
# As it says
func replace_column(a_column_index: int, a_new_column: Array[int]):
	...
	var t_column: Array[int] = a_new_column.duplicate()
	t_column.resize(size.y)
...
func out():
	...
	for i_row: Array[int] in t_rows:
		print_row(i_row)


# String representation of a_row
func print_row(a_row: Array[int]):
	var t_text: String = ""
	for i_item: int in a_row:
		t_text += str(i_item) + " "
	print(t_text)
...
func row_out_at_width(a_row: Array[int], a_width: int) -> String:
	var t_string: String = ""
	var t_spacer: String = ", "
	for i_item: int in a_row:
