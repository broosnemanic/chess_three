class_name Array2DAbstractSquare

var _internal: Array[AbstractSquare]			# Linear list of all items for internal use
var size: Vector2i						# [column count, row count]
var type: AbstractSquare						# For constructing null type and validating array arguments
...
func construct_from_rows(a_row_set: Array[Array]):
	...
	for i_row: Array in a_row_set:
		if not i_row is Array[AbstractSquare]: 
			return
	...
	for i_row_index: int in range(a_row_set.size()):
		# This assignment is to appease replace_row() which expects an Array[AbstractSquare]
		var t_row: Array[AbstractSquare] = a_row_set[i_row_index]
...
func construct_from_columns(a_column_set: Array[Array]):
	...
	for i_column: Array in a_column_set:
		if not i_column is Array[AbstractSquare]: 
			return
	...
	for i_column_index: int in range(a_column_set.size()):
		# This assignment is to appease replace_column() which expects an Array[AbstractSquare]
		var t_column: Array[AbstractSquare] = a_column_set[i_column_index]
...
func construct_from_linear(a_linear: Array[AbstractSquare], a_size: Vector2i):
...
# Clear and then load from a_linear without changing size
func load_linear_data(a_linear: Array[AbstractSquare]):
	var t_linear: Array[AbstractSquare] = a_linear.duplicate()
	t_linear.resize(_internal.size())
	for i_index: int in range(_internal.size()):
		_internal[i_index] = t_linear[i_index]


# Linear list of all items for external use
func linear() -> Array[AbstractSquare]:
	return _internal
...
func row(a_row_index: int) -> Array[AbstractSquare]:
...
func column(a_column_index: int) -> Array[AbstractSquare]:
...
func at(a_coord: Vector2i) -> AbstractSquare:
	return at_coord(a_coord.x, a_coord.y)


# Item at given column and row
func at_coord(a_column: int, a_row: int) -> AbstractSquare:
...
func append_row(a_row: Array[AbstractSquare]):
	...
	var t_row: Array[AbstractSquare] = a_row.duplicate()
	t_row.resize(size.x)
	size = Vector2i(size.x, size.y + 1)
	for i_item: AbstractSquare in t_row:
		_internal.append(i_item)


func append_column(a_column: Array[AbstractSquare]):
	# We are going to resize the added array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new column is too tall the extra items are ignored
	var t_column: Array[AbstractSquare] = a_column.duplicate()
	t_column.resize(size.y)
	var t_internal: Array[AbstractSquare] = []
...
func put(a_item: AbstractSquare, a_coord: Vector2i):
	put_at_coord(a_item, a_coord.x, a_coord.y)


# Replaces item at [a_column, a_row] with a_item
func put_at_coord(a_item: AbstractSquare, a_column: int, a_row: int):
	_internal[a_column + a_row * size.x] = a_item


# As it says
func replace_row(a_row_index: int, a_new_row: Array[AbstractSquare]):
	# We are going to resize the new row array to be sure we have the correct number
	# of items and do not want to alter the passed in array
	# Note this means if the new row is too long the extra items are ignored
	var t_row: Array[AbstractSquare] = a_new_row.duplicate()
	t_row.resize(size.x)
...
# As it says
func replace_column(a_column_index: int, a_new_column: Array[AbstractSquare]):
	...
	var t_column: Array[AbstractSquare] = a_new_column.duplicate()
	t_column.resize(size.y)
...
func out():
	...
	for i_row: Array[AbstractSquare] in t_rows:
		print_row(i_row)


# String representation of a_row
func print_row(a_row: Array[AbstractSquare]):
	var t_text: String = ""
	for i_item: AbstractSquare in a_row:
		t_text += i_item.str() + ", "
	print(t_text)
...
func row_out_at_width(a_row: Array[AbstractSquare], a_width: int) -> String:
	var t_string: String = ""
	var t_spacer: String = ", "
	for i_item: AbstractSquare in a_row:
		t_string += str(i_item).substr(0, a_width)
		t_string += t_spacer
	return t_string
